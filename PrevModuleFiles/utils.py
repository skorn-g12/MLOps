...
def check_if_table_has_value(cnx, table_name):
    ...
    check_table = pd.read_sql(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}';", cnx).shape[
        0]
    if check_table == 1:
        return True
    else:
    ...
    if process_flags['load_data'][0] == 1:
        if run_on == 'old':
            ...
            if append:
                ...
                march_user_logs, march_transactions = get_new_data_appended(old_data_directory, new_data_directory,
                                                                            start_data, end_date)
                ...
                if not check_if_table_has_value(cnx, 'transactions'):
                    ...
                    transactions['membership_expire_date'] = fix_time_in_df(transactions, 'membership_expire_date',
                                                                            expand=False)
...
def get_transaction_data_transform(db_path, db_file_name, drfit_db_name):
    ...
    if process_flags['process_transactions'][0] == 1:
        ...
        if not check_if_table_has_value(cnx, 'transactions_features_final'):
            ...
            transactions['amt_per_day'] = get_two_column_operations(transactions, 'actual_amount_paid',
                                                                    'payment_plan_days', "/")
            transactions['amt_per_day'] = get_replace_value_in_df(transactions, 'amt_per_day', [np.inf, -np.inf],
                                                                  replace_with=0)

            transactions['membership_duration'] = get_two_column_operations(transactions, 'membership_expire_date',
                                                                            'transaction_date', "-")
            transactions['membership_duration'] = get_timedelta_division(transactions, "membership_duration",
                                                                         td_type='D')
            transactions['membership_duration'] = get_convert_column_dtype(transactions, 'membership_duration',
                                                                           data_type='int')
            ...
            agg = {'payment_method_id': ['count', 'nunique'],
                   # How many transactions user had done in past, captures if payment method is changed
                   'payment_plan_days': ['mean', 'nunique'],
                   # Average plan of customer in days, captures how many times plan is changed
                   ...
                   'is_cancel': ['mean', 'max'],
                   # Captures the average value of is_cancel and to check if user changed its is_cancel state
                   'discount': 'mean',  # Average discount given to customer
                   'is_discount': ['mean', 'max'],
                   # Captures the average value of is_discount and to check if user was given any discount in the past
            ...
            transactions_features = get_groupby(transactions, by_column='msno', agg_dict=agg, agg_func='mean',
                                                simple_agg_flag=False, reset_index=True)
            transactions_features.columns = transactions_features.columns.get_level_values(
                0) + '_' + transactions_features.columns.get_level_values(1)
            transactions_features.rename(columns={'msno_': 'msno', 'payment_plan_days_nunique': 'change_in_plan',
                                                  ...
                                                  'transaction_date_count': 'total_transactions'}, inplace=True)
...
def get_user_data_transform(db_path, db_file_name, drfit_db_name):
    ...
    if process_flags['process_userlogs'][0] == 1:
        if not check_if_table_has_value(cnx, 'user_logs_features_final'):
            ...
            user_logs_transformed_base = get_groupby(user_logs_transformed, 'msno', agg_dict=None, agg_func='mean',
                                                     simple_agg_flag=True, reset_index=True)

            agg_dict = {'date': ['count', 'max']}
            user_logs_transformed_dates = get_groupby(user_logs_transformed, 'msno', agg_dict=agg_dict, agg_func='mean',
                                                      simple_agg_flag=False, reset_index=True)
...
def get_data_prepared_for_modeling(db_path, db_file_name, drfit_db_name, scale_method='standard', date_columns=None,
                                   corr_threshold=0.90, drop_corr=False,
...
def get_train_model_hptune(db_path, db_file_name, drfit_db_name):
    ...
    if process_flags['Model_Training_hpTunning'][0] == 1:
        ...
        gsearch = BayesSearchCV(estimator=lgb_estimator, search_spaces=gridParams, cv=gkf, n_iter=32, random_state=0,
...
def get_drift(old_data_directory, new_data_directory, db_path, drfit_db_name,
    ...
    # New Data
    march_user_logs, march_transactions = get_new_data_appended(old_data_directory, new_data_directory, start_data,
                                                                end_date)
    ...
    print("User Logs Data Drift as ", metric, " is: ",
          get_data_drift(march_user_logs, user_logs, column_list_userlogs, exclude_list_user_log, cnx, metric='std'))
    print("Transaction Data Drift as ", metric, " is: ",
          get_data_drift(march_transactions, transactions, column_list_tran, exclude_list_tran, cnx, metric='std'))
